# Jogo da velha

## Funcionalidades:
#### Modos de jogo: Jogar com um amigo ou com a máquina;
#### Validação de Dados: O sistema impede jogadas em casas ocupadas e valida se a entrada de texto é um número entre 1 e 3;
#### Detecção Automática: O jogo identifica vitória por linhas, colunas, diagonais ou empate ("Deu Velha");
#### Loop de Partida: Opção de reiniciar o jogo sem precisar fechar o programa;

## Como jogar:
#### O tabuleiro é baseado em coordenadas de Linha (1 a 3) e Coluna (1 a 3);
#### Ao iniciar, escolha o modo de jogo (0 para Máquina ou 1 para Amigo);
#### No seu turno, digite o número da Linha e pressione Enter;
#### Digite o número da Coluna e pressione Enter;
#### O objetivo é completar uma linha, coluna ou diagonal com o seu símbolo (X ou O);
